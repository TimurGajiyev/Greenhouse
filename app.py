...
make_t = _i18n.make_t
# Язык-зависимые блоки берём функциями из каталога: добавление нового
# языка не требует правок здесь (список языков — тоже оттуда).
LANGUAGES = _i18n.LANGUAGES
get_glossary, get_columns_help = _i18n.get_glossary, _i18n.get_columns_help
...
@st.cache_data(show_spinner="Подбираю оптимальную конфигурацию...")
...
@st.cache_data(show_spinner="Готовлю профили нагрузки и солнца...")
...
@st.cache_data(show_spinner="Проверяю план в реальном режиме...")
...
@st.cache_data(show_spinner="Ищу альтернативные конфигурации...")
...
@st.cache_data(show_spinner="Считаю риски и чувствительность...")
...
lang = st.sidebar.segmented_control(
    "Язык / Language", list(LANGUAGES), default="RU", key="lang",
...
DISCLAIMER = {
    ...
    "EN": "The model is a tool for understanding and checking vendor "
          ...
          "design.",
    "CS": "Model je nástroj pro pochopení a ověření nabídek dodavatelů, "
          "NIKOLI náhrada autorizovaného inženýra pro finální projekt.",
}[lang]
...
with st.sidebar.form("params"):
    ...
    fuel_price_l = st.slider(
        ...
        # (внутри: цена за объём — фундаментальный вход REopt/HOMER)
        help=T("Цена одного литра дизтоплива на площадке (с доставкой). "
               "Фундаментальный вход отраслевых моделей; $/кВт*ч "
               "выводится из неё и удельного расхода. Анализ рисков "
    ...
    fuel_esc_pct = st.slider(
        ...
        # (внутри: свёрнуто в один левелизационный коэффициент — приём
        #  pwf_fuel REopt, благодаря которому задача остаётся линейной)
        help=T("Насколько цена топлива растёт каждый год сверх инфляции. "
               ...
               "коэффициентом — расчёт остаётся быстрым."))

    st.subheader(T("PV-модуль и инвертор (datasheet)"))
    inv_eff = st.slider(
        T("КПД инвертора"), 0.90, 0.99, 0.96, 0.005,
        # (внутри: 0.96 — дефолт REopt/PVWatts)
        help=T("Номинальный КПД DC→AC. Дефолт 0.96 (отраслевой "
               "стандарт); в datasheet вендора обычно 0.95–0.985."))
    gamma_pct = st.slider(
    ...
    mount = st.selectbox(
        ...
        # (внутри: значимость подтвердил верификационный кейс NIST)
        help=T("Влияет на температуру ячейки: на раме панели охлаждаются "
               "лучше (+1–2% выработки). Подтверждено полевыми замерами."))
    ...
    voll = st.number_input(
        ...
        # (внутри: дефолт $1/kWh — value_of_lost_load_per_kwh REopt)
        help=T("Value of lost load — цена недопоставленного kWh для "
               "потребителя (простой производства). Отраслевой дефолт: $1."))
    ...
    cc_on = st.checkbox(
        ...
        # (внутри: стратегия Cycle Charging из HOMER)
        help=T("Стратегия «подзаряд от генсета»: раз генсет уже работает, "
    ...
    cyclic = st.checkbox(
        ...
        # (внутри: паттерн cyclic_storage Calliope; выключение = стиль
        #  REopt off-grid, старт с полной батареей)
        help=T("Запас батареи в конце года «перетекает» в его начало — "
               "система не получает бесплатной стартовой заправки. "
               "Выключи, если нужен старт с полной батареей."))
...
with tab_cfg:
    ...
    with st.expander(T("Что означает каждая колонка")):
        st.markdown(get_columns_help(lang))
...
with tab_res:
    ...
    # (внутри: спутниковый TMY из PVGIS, физика панели — модель PVWatts)
    tab_footer(
        "Это два входных ряда, из которых следует всё остальное: спрос "
        "(нагрузка по часам) и предложение (выработка 1 kWp из "
        "спутникового «типичного года» по стандартной отраслевой модели "
        "солнечной выработки). Любое странное число на других вкладках "
        "сначала проверяют здесь."
    )
...
with tab_scen:
    ...
    # в интерфейсе метод не называем — клиенту важен результат.
    st.subheader(T("Альтернативные конфигурации"))
    ...
    tab_footer(
        ...
        "вариантами; раздел альтернатив добавляет к нему веер «другого "
        "железа за почти те же деньги»."
    )

# ---------- валидация ----------

with tab_valid:
    # (внутри: сверялись с REopt web и HOMER; публичного API у HOMER нет,
    #  у REopt нужен ключ NREL — поэтому числа вводятся вручную)
    st.markdown(T(
        "Сверка с внешним инструментом. Прогони тот же сценарий в любом "
        "независимом калькуляторе и впиши его числа — отклонения "
        "**> 10%** будут помечены. Числа вводятся вручную: так результат "
        "остаётся проверяемым и не зависит от чужих сервисов."))
    cc = st.columns(4)
    ref = {
    ...
    # Проведённые сверки (для истории проекта, в интерфейсе не называем):
    # Тонга — в диапазоне HOMER (LCOE $0.27 при 0.25-0.32); DeGrussa —
    # против фактов ARENA (расхождение объяснено трекерами); PV-цепочка —
    # против NREL SAM и датчиков в Оклахоме (+-2%); полигон NIST — показал,
    # что параметры модуля должны быть полями схемы (теперь они в форме).
    tab_footer(
        "Внешний контроль качества: тот же сценарий в независимом "
        "инструменте, его числа — сюда, отклонение до 10% — нормальный "
        "разброс допущений. Модель уже прошла серию независимых сверок: "
        "против отраслевых калькуляторов, против фактических данных "
        "действующих станций и против показаний датчиков на площадках — "
        "все расхождения объяснены и задокументированы."
    )
